...
def grounding_hash(body, all_vars):
    cons = frozenset(
        (lit.predicate, lit.arguments) for lit in body if lit.predicate in META_PREDS
    )
...
class Generator:
    ...
    def get_ground_rules(self, rule):
        ...
        return set(
            self.ground_rule((head, body), assignment) for assignment in assignments
        )
        ...
        for xs in tmp_new_cons:
            ...
            if con_type == Constraint.GENERALISATION:
                ...
                if self.settings.noisy and len(xs) > 2:
                    con_size = xs[2]
                new_rule_handles2, con = self.build_generalisation_constraint2(
                    con_prog, gen_size=con_size
                )
                ...
                if self.settings.noisy and len(xs) > 2:
                    con_size = xs[2]
                new_rule_handles2, con = self.build_specialisation_constraint2(
                    con_prog, spec_size=con_size
                )
                self.all_handles.update(new_rule_handles2)
                new_cons.add(con)
                ...
                bad_handle, new_rule_handles2, con = self.redundancy_constraint1(
                    con_prog
                )
    ...
    def make_rule_handle(self, rule):
        ...
        handle = "".join(
            f'{literal.predicate}{"".join(map(str,literal.arguments))}'
            for literal in [head] + body_literals
        )
    ...
    def build_seen_rule2(self, rule, is_rec):
        ...
        body_vars = set(
            x for atom in body for x in atom.arguments if x not in head_vars
        )
            ...
            for xs in perms:
                ...
                for atom in body:
                    ...
                    new_body.append(
                        (
                            "body_literal",
                            (rule_id, atom.predicate, len(atom.arguments), new_args),
                        )
                    )
    ...
    def redundancy_constraint1(self, prog):
        ...
        literals.append(gteq(rule_var, 1))
        literals.append(
            Literal("recursive_clause", (rule_var, head.predicate, len(head.arguments)))
        )
                ...
                for atom in body:
                    ...
                    rule.append(
                        (
                            True,
                            "body_literal",
                            (rule_id, atom.predicate, len(atom.arguments), args2),
                        )
                    )
