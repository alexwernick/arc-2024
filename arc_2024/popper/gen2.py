...
class Generator:
        ...
        for xs in tmp_new_cons:
            ...
            if con_type == Constraint.GENERALISATION or con_type == Constraint.BANISH:
                ...
                if self.settings.noisy and len(xs) > 2:
                    con_size = xs[2]
                ground_rules2 = tuple(
                    self.build_generalisation_constraint3(con_prog, con_size)
                )
                ...
                if self.settings.noisy and len(xs) > 2:
                    con_size = xs[2]
                ground_rules2 = tuple(
                    self.build_specialisation_constraint3(con_prog, con_size)
                )
                new_ground_cons.update(ground_rules2)
    ...
    def find_variants(self, rule, max_rule_vars=False):
        head, body = rule
        body_vars = frozenset(
            x for literal in body for x in literal.arguments if x >= len(head.arguments)
        )
        if max_rule_vars:
