...
class Tester:
    def __init__(self, settings):
        ...
        if not settings.pi_enabled:
            consult(
                "prog",
                f":- dynamic {settings.head_literal.predicate}/{len(settings.head_literal.arguments)}.",
            )
        ...
        self.num_pos = query_once(
            "findall(_K, pos_index(_K, _Atom), _S), length(_S, N)"
        )["N"]
        self.num_neg = query_once(
            "findall(_K, neg_index(_K, _Atom), _S), length(_S, N)"
        )["N"]
    ...
    def is_sat(self, prog):
        if len(prog) == 1:
            ...
            with self.using(prog):
                if self.settings.noisy:
                    return query_once(
                        f"covers_at_least_k_pos(K)", {"K": calc_prog_size(prog)}
                    )["truth"]
                else:
