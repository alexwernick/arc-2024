...
class Popper:
    ...
    def run(self, bkcons):
        ...
        combiner = load_solver(
            settings, tester, coverage_pos, coverage_neg, prog_lookup
        )
        ...
        for size in range(2, max_size + 1):
            ...
            while True:
                    ...
                    if settings.noisy:
                        if settings.recursion_enabled or settings.pi_enabled:
                            ...
                            if tp > prog_size:
                                ...
                                test_at_most_k_neg1 = min(
                                    [
                                        settings.max_body - (prog_size - 1),
                                        settings.max_literals - prog_size,
                                    ]
                                )
                                # conditions which determine whether a program can be part of a solution
                                test_at_most_k_neg2 = min(
                                    [settings.best_mdl - prog_size, tp - prog_size]
                                )
                                test_at_most_k_neg = max(
                                    [test_at_most_k_neg1, test_at_most_k_neg2]
                                )
                                neg_covered = tester.test_single_rule_neg_at_most_k(
                                    prog, test_at_most_k_neg
                                )
                ...
                if (
                    not skipped and not inconsistent and tp == num_pos
                ):  # and not settings.order_space:
                    if not settings.functional_test or not tester.is_non_functional(
                        prog
                    ):
                    ...
                    if not skipped:
                        ...
                        if settings.debug:
                            settings.logger.debug(
                                f"tp:{tp} fn:{fn} tn:{tn} fp:{fp} mdl:{mdl}"
                            )
                ...
                if (
                    ...
                    with settings.stats.duration("check subsumed and covers_too_few"):
                        ...
                        subsumed_by_two = not subsumed and self.subsumed_by_two_new(
                ...
                if (
                    ...
                    subsumed_progs = []
                    with settings.stats.duration(
                        "find most general subsumed/covers_too_few"
                    ):
                        subsumed_progs = self.subsumed_or_covers_too_few(
                            prog, seen=set()
                        )
                ...
                if add_to_combiner:
                    ...
                    if not settings.noisy and not has_invention:
                        with settings.stats.duration("prune backtrack subsumed"):
                            subsumed_progs = tuple(
                                self.prune_subsumed_backtrack(pos_covered, prog_size)
                            )
                            for subsumed_prog_ in subsumed_progs:
                                subsumed_prog_ = frozenset(
                                    remap_variables(rule) for rule in subsumed_prog_
                                )
                                new_cons.append(
                                    (Constraint.SPECIALISATION, subsumed_prog_)
                                )

                call_combine = len(to_combine) > 0
                call_combine = call_combine and (
                    settings.noisy or settings.solution_found
                )
                call_combine = call_combine and (
                    ...
                    if any_and(uncovered, pos_covered):
                        ...
                        if not uncovered.any():
                            ...
                            if not settings.noisy:
                                with settings.stats.duration(
                                    "prune backtrack covers too few"
                                ):
                                    subsumed_progs = tuple(
                                        self.prune_subsumed_backtrack_specialcase()
                                    )
                                    for subsumed_prog_ in subsumed_progs:
                                        subsumed_prog_ = frozenset(
                                            remap_variables(rule)
                                            for rule in subsumed_prog_
                                        )
                                        new_cons.append(
                                            (Constraint.SPECIALISATION, subsumed_prog_)
                                        )
                    ...
                    if (
                        ...
                        if spec_size <= settings.max_literals and (
                            (
                                is_recursive
                                or has_invention
                                or spec_size <= settings.max_body
                            )
                        ):
                            new_cons.append(
                                (Constraint.SPECIALISATION, prog, spec_size)
                            )
                ...
                if (
                    ...
                    and not add_gen
                    and gen_size
                    and not pruned_sub_inconsistent
                ):
                    if (
    ...
    def check_covers_too_few(self, prog_size, pos_covered):
        ...
        # # MAX RULES = 3
            ...
            success_sets = sorted(
                (
                    (pos_covered_, size)
                    for (pos_covered_, size) in self.success_sets.items()
                ),
                key=lambda x: x[1],
            )
            n = len(success_sets)
    ...
    def prune_subsumed_backtrack(self, pos_covered, prog_size):
        could_prune_later, could_prune_later_rec, tester, settings = (
            self.could_prune_later,
            self.could_prune_later_rec,
            self.tester,
            self.settings,
        )
        to_prune = set()
        ...
        for index, (prog2, pos_covered2, prog2_size) in enumerate(
            could_prune_later_rec
        ):
        ...
        for index, (prog2, pos_covered2, prog2_size) in enumerate(could_prune_later):
            ...
            for new_body in non_empty_subset(body):
                ...
                sub_subsumed_by_two = (
                    not sub_prog_subsumed
                    and self.subsumed_by_two_new(
                        sub_prog_pos_covered, calc_prog_size(new_prog)
                    )
                )
                ...
                if self.settings.showcons:
                    if sub_prog_subsumed:
                        print(
                            "\t",
                            "SUBSUMED BACKTRACK (GENERALISATION):",
                            "\t",
                            format_prog(new_prog),
                        )
                    elif sub_subsumed_by_two:
                        print(
                            "\t",
                            "SUBSUMED BY TWO BACKTRACK (GENERALISATION):",
                            "\t",
                            format_prog(new_prog),
                        )
    ...
    def prune_subsumed_backtrack_specialcase(self):
        could_prune_later, tester, settings = (
            self.could_prune_later,
            self.tester,
            self.settings,
        )
        ...
        for index, (prog2, pos_covered2, prog2_size) in enumerate(could_prune_later):
            ...
            covers_too_few = self.check_covers_too_few(
                calc_prog_size(prog2), pos_covered2
            )
            ...
            for new_body in non_empty_subset(body):
                ...
                sub_prog_pos_covered = tester.get_pos_covered(new_prog)
                sub_covers_too_few = self.check_covers_too_few(
                    calc_prog_size(new_prog), sub_prog_pos_covered
                )
                ...
                if sub_covers_too_few:
                    if self.settings.showcons:
                        print(
                            "\t",
                            "COVERS TOO FEW BACKTRACK (GENERALISATION)",
                            "\t",
                            format_prog(new_prog),
                        )
                    to_prune.add(new_prog)
    ...
    def explain_totally_incomplete_aux2(self, prog, unsat2=set(), unsat=set()):
        ...
        out = []
        for subprog in generalisations(
            prog, allow_headless=True, recursive=has_recursion
        ):
            # print('---')
            # for rule in subprog:
    ...
    def has_valid_directions(self, rule):
        ...
        if head:
            ...
            # literal_inputs =
            if all(
                len(self.settings.literal_inputs[(pred, args)]) == 0
                for pred, args in body
            ):
                return True
            ...
            while body_literals:
                ...
                grounded_variables = grounded_variables.union(
                    selected_literal.arguments
                )
                body_literals = body_literals.difference({selected_literal})
    ...
    def print_incomplete_solution2(self, prog, tp, fn, tn, fp, size):
        ...
        if self.noisy:
            self.logger.info(
                f"tp:{tp} fn:{fn} tn:{tn} fp:{fp} size:{size} mdl:{size+fn+fp}"
            )
        else:
