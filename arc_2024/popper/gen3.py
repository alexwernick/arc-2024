...
from .util import Constraint, Literal, bias_order, rule_is_recursive
...
class Generator:
    ...
    def make_rule_handle(self, rule):
        ...
        handle = "".join(
            f'{pred}{"".join(map(str, args))}' for pred, args in [head] + body_literals
        )
        ...
        for xs in permutations(subset, len(body_vars)):
            ...
            for pred, args in body:
                ...
                new_literal = (
                    True,
                    "body_literal",
                    (ruleid, pred, len(new_args), new_args),
                )
        ...
        for rule_id in range(self.settings.max_rules):
            ...
            for xs in perms:
                ...
                for pred, args in body:
                    ...
                    new_body.append(
                        ("body_literal", (rule_id, pred, len(args), new_args))
                    )
        ...
        for rule_id in range(self.settings.max_rules):
            for assignment in assignments:
                ...
                for pred, args in body:
                    ...
                    rule.append(
                        (True, "body_literal", (rule_id, pred, len(args), args2))
                    )
