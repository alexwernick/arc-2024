...
def old_wcnf_to_file(hard_clauses, soft_clauses, weights, file):
    ...
    for clause in soft_clauses:
        n_vars = max(n_vars, max([abs(lit) for lit in clause]))
    n_clauses = (
        len(hard_clauses) + len(soft_clauses) - len([w for w in weights if w == 0])
    )
    top = sum(weights) + 1
    ...
    if settings.exact_maxsat_solver == "rc2":
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".wcnf") as tmp:
            ...
            try:
                ...
                args = (
                    [settings.exact_maxsat_solver]
                    + settings.exact_maxsat_solver_params.split()
                    + [tmp.name]
                )
                # print(args)
        ...
        if "s UNSATISFIABLE" in output:
            ...
            model = [
                i if model_line[i - 1] == "1" else -i
                for i in range(1, len(model_line) + 1)
            ]
            return cost, model
        else:
            ...
            try:
                ...
                args = (
                    [
                        "timeout",
                        "-s",
                        str(settings.anytime_maxsat_solver_signal),
                        str(timeout),
                        settings.anytime_maxsat_solver,
                    ]
                    + settings.anytime_maxsat_solver_params.split()
                    + [tmp.name]
                )
                # print(args)
                output = subprocess.check_output(args).decode("utf-8").split("\n")
...
def exact_lex_solve(hard_clauses, soft_lit_groups, last_weights, settings):
    ...
    if not settings.lex_via_weights:
        ...
        for soft_lits in soft_lit_groups[:-1]:
            i = i + 1
            cost, model = exact_maxsat_solve(
                hard_clauses,
                [[lit] for lit in soft_lits],
                [1 for _ in soft_lits],
                settings,
            )
            if model is None:
                return cost, model
            card = CardEnc.atmost(
                [-lit for lit in soft_lits], bound=cost, top_id=top_id, encoding=1
            )
            for clause in card.clauses:
                hard_clauses.append(clause)
                top_id = max(top_id, max([abs(lit) for lit in clause]))
        cost, model = exact_maxsat_solve(
            hard_clauses, [[lit] for lit in soft_lit_groups[-1]], last_weights, settings
        )
        return cost, model
    else:
...
def anytime_lex_solve(hard_clauses, soft_lit_groups, last_weights, settings, timeout):
    ...
    if not settings.lex_via_weights:
        ...
        for clause in hard_clauses:
            top_id = max(top_id, max([abs(lit) for lit in clause]))
        for soft_lits in soft_lit_groups[:-1]:
            cost, model = anytime_maxsat_solve(
                hard_clauses,
                [[lit] for lit in soft_lits],
                [1 for _ in soft_lits],
                settings,
            ...
            for clause in card.clauses:
                hard_clauses.append(clause)
                top_id = max(top_id, max([abs(lit) for lit in clause]))
        return anytime_maxsat_solve(
            hard_clauses,
            [[lit] for lit in soft_lit_groups[-1]],
            last_weights,
            settings,
            timeout,
        )
