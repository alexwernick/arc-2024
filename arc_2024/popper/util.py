...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="tell Popper that there is noise",
    )
...
class Settings:
    def __init__(
        ...
        solver="rc2",
        anytime_solver=None,
        anytime_timeout=ANYTIME_TIMEOUT,
    ):
        if cmd_line:
            args = parse_args()
        ...
        if quiet:
            ...
            logging.basicConfig(
                format="%(message)s", level=log_level, datefmt="%H:%M:%S"
            )
        elif info:
            log_level = logging.INFO
            logging.basicConfig(
                format="%(asctime)s %(message)s", level=log_level, datefmt="%H:%M:%S"
            )
        ...
        if self.has_directions:
            ...
            for head_args in permutations(range(self.max_vars), len(head_args)):
                head_inputs = frozenset(
                    arg
                    for i, arg in enumerate(head_args)
                    if directions[head_pred][i] == "+"
                )
                head_outputs = frozenset(
                    arg
                    for i, arg in enumerate(head_args)
                    if directions[head_pred][i] == "-"
                )
...
def bias_order(settings, max_size):
    if not (settings.no_bias or settings.order_space):
        return [
            (size_literals, settings.max_vars, settings.max_rules, None)
