...
class Combiner:
    ...
    def find_combination(self, timeout):
        ...
        if self.settings.noisy:
            for i in neg_index:
                neg_example_covered_var[i] = vpool.id(
                    "neg_example_covered({0})".format(i)
                )
        ...
        for program_count, prog_hash in enumerate(self.saved_progs):
            ...
            if len(rule_vars) == 1:
                ...
                program_var[program_count] = vpool.id(
                    "program({0})".format(program_count)
                )
        ...
        for ex in pos_index:
            encoding.append(
                [-pos_example_covered_var[ex]]
                + [program_var[p] for p in programs_covering_pos_example[ex]]
            )
        ...
        while True:
            ...
            if not self.settings.lex:
                if timeout is None or self.settings.last_combine_stage:
                    cost, model = maxsat.exact_maxsat_solve(
                        encoding, soft_clauses, weights, self.settings
                    )
                else:
                    cost, model = maxsat.anytime_maxsat_solve(
                        encoding, soft_clauses, weights, self.settings, timeout
                    )
            else:
                if timeout is None or self.settings.last_combine_stage:
                    cost, model = maxsat.exact_lex_solve(
                        encoding, soft_lit_groups, weights, self.settings
                    )
                else:
                    cost, model = maxsat.anytime_lex_solve(
                        encoding, soft_lit_groups, weights, self.settings, timeout
                    )
            ...
            if not self.settings.nonoise:
                fp = sum(
                    1 for i in neg_index if model[neg_example_covered_var[i] - 1] > 0
                )
            size = sum(
                [
                    ruleid_to_size[rule_id]
                    for rule_id in ruleid_to_size
                    if model[rule_var[rule_id] - 1] > 0
                ]
            )
            ...
            if self.settings.lex:
                if self.settings.best_prog_score:
                    if (
                        fn_ < fn
                        or (fn_ == fn and fp_ < fp)
                        or (fn_ == fn and fp_ == fp and size_ <= size)
                    ):
                        break
            else:
                if self.settings.best_prog_score:
            ...
            if not prog_is_recursive(model_prog) and not prog_has_invention(model_prog):
                ...
                if not self.settings.lex:
                    print(
                        "ERROR: Combining rec or pi programs not supported with MDL objective. Exiting."
                    )
                    assert False
